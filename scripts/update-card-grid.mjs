...
const TARGETS = ['Website', 'UGC_Unboxing']
  ...
  for (const folder of TARGETS) {
    ...
    for (const filename of files) {
      ...
      try {
        ...
        console.log('OK')
        if (isMain) results[slug].main = { url: result.secure_url, type: resourceType }
        else results[slug].gallery.push({ url: result.secure_url, type: resourceType, filename })
      } catch (err) {
        console.log(`FAIL ${err.message}`)
      }
    }
  }
  fs.writeFileSync(path.join(__dirname, 'card-grid-update-result.json'), JSON.stringify(results, null, 2))
  console.log('\nResults -> scripts/card-grid-update-result.json')
}
